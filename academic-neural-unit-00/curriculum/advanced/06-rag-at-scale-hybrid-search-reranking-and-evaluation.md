...
completed [`intermediate/06` — RAG Fundamentals: Retrieval, Embeddings & Grounding](https://anu00.dev/curriculum/intermediate/06-rag-fundamentals-retrieval-embeddings-and-grounding.md) will find this chapter's Section 1 a light, familiar recap rather than
new information.
...
[`advanced/05` — Advanced Context Engineering: Long-Context & Context Budgeting](https://anu00.dev/curriculum/advanced/05-advanced-context-engineering-long-context-and-budgeting.md) established that a context window is a finite, expensive resource whose middle region
...
into a single prompt — which, per [`advanced/05` — Advanced Context Engineering: Long-Context & Context Budgeting §1](https://anu00.dev/curriculum/advanced/05-advanced-context-engineering-long-context-and-budgeting.md#1-recap-and-scope)'s $O(n^2)$ cost and [§8](#8-common-failure-modes-at-scale)'s budgeting discipline,
...
[`introductory/02` — The Transformer Architecture & Attention §3](https://anu00.dev/curriculum/introductory/02-the-transformer-architecture-and-attention.md#3-the-core-idea-of-attention-letting-every-position-look-at-every-other-position) already defined an embedding vector as a learned, fixed-size numeric
...
mechanism ([`introductory/02` — The Transformer Architecture & Attention §4](https://anu00.dev/curriculum/introductory/02-the-transformer-architecture-and-attention.md#4-queries-keys-and-values)–[§6](#6-assembling-a-production-rag-pipeline)) directly compare query tokens against passage tokens within the
...
context-budgeting pitfalls in [`advanced/05` — Advanced Context Engineering: Long-Context & Context Budgeting §9](https://anu00.dev/curriculum/advanced/05-advanced-context-engineering-long-context-and-budgeting.md#9-common-pitfalls-at-this-level).
...
directly to [`advanced/05` — Advanced Context Engineering: Long-Context & Context Budgeting](https://anu00.dev/curriculum/advanced/05-advanced-context-engineering-long-context-and-budgeting.md)'s context-budgeting discipline; and reference-free evaluation via RAGAS's
...
Together with [`advanced/05` — Advanced Context Engineering: Long-Context & Context Budgeting](https://anu00.dev/curriculum/advanced/05-advanced-context-engineering-long-context-and-budgeting.md), this chapter completes the Prompt & Context Engineering cluster's
...
- [`introductory/02` — The Transformer Architecture & Attention](https://anu00.dev/curriculum/introductory/02-the-transformer-architecture-and-attention.md) — prerequisite: embedding vector definition, attention mechanism used by cross-encoders in [§5](#5-reranking-cross-encoders-and-late-interaction).
- [`introductory/06` — Context Windows, Tokens & Memory Basics](https://anu00.dev/curriculum/introductory/06-context-windows-tokens-and-memory-basics.md) — prerequisite: token and context-window vocabulary.
