# Harness Configuration Specification — Mobile Development Pipeline
...
This document defines the harness configuration that **every executor agent** operating in the Mobile Development Pipeline must apply. These are not suggestions — missing any Mandatory item is a P0 compliance gap that blocks production readiness.
...
| Critical   | Irreversible ops (production release, DB migration) | **Yes — explicit User approval** |
...
- [ ] Timeouts configured per §1 for all call types
