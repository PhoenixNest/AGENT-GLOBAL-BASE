...
2. IDS includes API UX specifications for developer portal (documentation layout, interactive explorer, error response design, onboarding flow)
3. IDS includes WCAG 2.1 AA accessibility targets for developer portal (screen reader compatibility, keyboard navigation, color contrast)
4. Interactive API mock server or Swagger UI instance demonstrates all endpoints with sample responses
5. CDO confirms API design fidelity to PRD requirements
...
- 8 mandatory ADRs:
  - `ADR-API-STRATEGY.md` -- REST vs GraphQL vs gRPC decision (14 fields + compliance implications, including data consistency model)
...
1. All 8 ADRs + TSD authored, reviewed, and approved
2. TSD includes weighted scorecard with at least 2 alternatives per technology decision
3. UML diagrams cover data layer to domain layer to API layer
...
2. API Design Conformance Review passed at ~60% completion (OpenAPI/Swagger spec matches implementation exactly; minor gaps (less than or equal to 3 undocumented endpoints/fields) proceed with remediation plan; major gaps (>3) STOP, CTO notifies CPO)
3. Developer Portal UI Conformance Check passed at ~60% completion (≥85% completeness on documentation layout, interactive explorer, error response design, onboarding flow against IDS; <85% proceed with documented remediation plan)
4. String Extraction Readiness audit completed -- remaining hardcoded strings less than or equal to 5%, classified as P2 (P1 if core API error affected)
5. Pact Contract Verification Reports produced at 30% and 70% milestones
6. CTO internal review checklist completed:
...
1. Three-Layer Defense for ADR/TSD compliance passed:
   ...
   - **Layer 3:** CI/CD gates passed -- dependency version pinning, prohibited technology detection, security ADR compliance (rate limiting, authZ patterns)
