# Harness Configuration Specification — Full-Stack Pipeline
...
This document defines the harness configuration that **every executor agent** operating in the Full-Stack Pipeline must apply. These are not suggestions — missing any Mandatory item is a P0 compliance gap that blocks production readiness.
...
| Cross-platform sync call |  25 s   | Log `TimeoutError` → alert VP API and VP Web |

> Never use a blanket catch-all timeout. Each call type has its own timeout and recovery action.
> The full-stack pipeline coordinates mobile + web + backend agents — timeouts must be enforced
> independently per sub-system; do not aggregate timeouts across stacks.
...
>
> **Full-stack note:** Multi-stack coordination (mobile + web + backend simultaneously) will
> consume context faster. Monitor at 60% when running all three sub-system agents in parallel.
...
| High       | Deploy, send external comms, delete records                                |       **Yes — User gate**        |
| Critical   | Irreversible ops (production release, DB migration, multi-platform deploy) | **Yes — explicit User approval** |
...
| Step                       | Action                                                                 |
| :------------------------- | :--------------------------------------------------------------------- |
| Before LLM call            | Scrub all PII from input (names, emails, device IDs, API keys, tokens) |
| After LLM response         | Scan output for PII patterns before passing to downstream stages       |
| Cross-platform handoff     | Re-scrub at every API boundary between mobile ↔ web ↔ backend agents   |
| On PII detection in output | Redact → log incident → notify CIO Dr. Priya Mehta                     |

> PII in logs is a P0 defect. Zero-tolerance — no exceptions.
...
- [ ] Timeouts configured per §1 for all call types (including cross-platform sync timeout)
- [ ] Error boundary handles all 5 error types by name (no blanket catch)
- [ ] 429 retry with exponential backoff + jitter is active (§3)
- [ ] Token budget monitor thresholds set at 70%/85%/90% with 60% early-warning in multi-stack mode (§4)
- [ ] Tool registry whitelist loaded for current stage (§5)
- [ ] PII scrubber applied at every cross-platform boundary (§6)
