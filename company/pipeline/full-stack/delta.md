...
> **Meta-pipeline note.** Full-stack is unique among deltas: it is a **meta-pipeline** that orchestrates coordinated delivery across [`company/pipeline/mobile-development/delta.md`](company/pipeline/mobile-development/delta.md), [`company/pipeline/web-development/delta.md`](company/pipeline/web-development/delta.md), and [`company/pipeline/backend-api/delta.md`](company/pipeline/backend-api/delta.md). It does **not** replace those per-platform deltas — it sits above them and adds cross-platform orchestration concerns (release coordination, parity verification, shared data models, unified auth, design-token pipeline). When a project chooses `full-stack`, it activates this delta **plus** the relevant per-platform deltas in tandem. The base + full-stack delta + per-platform deltas together produce the derived view.
...
**Ownership:** CTO authors. All platform leads (Frontend, Backend, Mobile) provide input. CIO reviews for technology conformance. CSO reviews for security conformance. CDO reviews for design quality impact. The ADR is versionable + supersedable per [`company/pipeline/_base/adr-template.md`](company/pipeline/_base/adr-template.md); supersession requires a documented rollback plan and triggers an Implementation-Plan re-baseline (Stage 4 re-entry minimum).
...
- **Track FS-WFE (Web Frontend):** inherits engineering execution from [`company/pipeline/web-development/delta.md`](company/pipeline/web-development/delta.md) Track W-FE. FULL for full-product and web+API; dormant for mobile+API.
- **Track FS-WBE (Web Backend):** inherits engineering execution from [`company/pipeline/backend-api/delta.md`](company/pipeline/backend-api/delta.md) Track B-API + B-DATA. FULL for all three full-stack scenarios.
- **Track FS-MOB (Mobile):** inherits engineering execution from [`company/pipeline/mobile-development/delta.md`](company/pipeline/mobile-development/delta.md) (its own Mobile Platform Strategy Matrix applies underneath this delta). FULL for full-product and mobile+API; dormant for web+API.
...
- **Per-platform deployment sub-checklists confirmed:**
  - **Web:** Vercel deploy + CDN + DNS + SSL verified (per [`company/pipeline/web-development/delta.md`](company/pipeline/web-development/delta.md)).
  - **Mobile:** App Store + Google Play submitted + listings verified (per [`company/pipeline/mobile-development/delta.md`](company/pipeline/mobile-development/delta.md)).
  - **Backend:** API gateway live + sandbox verified + status page live (per [`company/pipeline/backend-api/delta.md`](company/pipeline/backend-api/delta.md)).
