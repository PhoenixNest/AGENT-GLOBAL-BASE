...
description: Pact contract testing for mobile-backend API verification — consumer-driven contract tests, provider verification, pact broker integration, and API compatibility validation for Android, iOS, and KMP shared module consumers. Owned by Rachel Kim (Test Automation Lead). Use during Stage 5 (Development) for contract test implementation and Stage 7 (Testing) for API compatibility verification. Trigger: Pact contract testing, consumer-driven contracts, provider verification, pact broker, API compatibility, mobile API contracts, KMP contract tests.
prerequisites:
  - testing-qa-overview
