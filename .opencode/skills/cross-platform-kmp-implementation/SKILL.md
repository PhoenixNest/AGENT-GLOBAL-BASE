---
name: cross-platform-kmp-implementation
description: Kotlin Multiplatform (KMP) implementation — shared module architecture, expect/actual patterns, Ktor shared networking, SQLDelight database, Swift interoperability, Kotlin/Native memory model, Gradle multi-module KMP build. Owned by Mei-Ling Johansson (Cross-Platform Lead). Use during Stage 5 (Development) for KMP shared module implementation and Stage 6 (Code Review) for KMP conformance. Trigger: KMP implementation, Kotlin Multiplatform coding, expect/actual, Ktor networking, SQLDelight, Kotlin/Native.
prerequisites:
