...
description: API testing methodology for REST and GraphQL services — authentication validation (OAuth 2.0, JWT), rate limiting verification, error handling resilience, and performance benchmarking. Owned by Dev Malhotra (Backend Chapter Lead). Use during Stage 7 (Automated Testing) for API test infrastructure and Stage 6 (Code Review) for API contract validation. Trigger: api testing, rest testing, graphql testing, oauth testing, jwt validation, rate limit testing, api performance, contract testing.
prerequisites:
  - backend-overview
...
**Category:** Backend & API Test Engineering
**Owner:** SDET Web/Backend #1 (Priya Sharma)

## Overview

This skill covers comprehensive API testing methodology spanning REST and GraphQL services, authentication and authorization validation (OAuth 2.0, JWT), rate limiting and throttling verification, error handling resilience, and performance benchmarking. API testing forms the integration layer of the test pyramid — faster and more deterministic than UI tests while providing broader coverage than unit tests.

API testing is critical for Stage 7 automated testing of backend services, microservice contracts, and the API surface that mobile clients depend on. Thorough API validation ensures that mobile applications receive correct, secure, and performant responses under all operational conditions.
