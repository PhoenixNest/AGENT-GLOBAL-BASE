---
name: android-security-ci-cd-security-basics
description: Android security baseline — OWASP MASVS L1 controls, EncryptedSharedPreferences, certificate pinning, input validation, deep link security, FLAG_SECURE screenshot protection, and security hygiene (debuggable=false, log sanitization). Owned by Kwame Osei (Android Engineer). Use during Stage 5 (Development) for baseline security implementation and Stage 6 (Code Review) for security conformance checks. Trigger: android security basics, MASVS L1, secure storage, certificate pinning, input validation, deep link security, FLAG_SECURE, security hygiene.
prerequisites:
  - android-security-ci-cd-security
