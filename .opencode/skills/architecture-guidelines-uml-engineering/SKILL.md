...
description: UML engineering for mobile architecture — class diagrams, sequence diagrams, component diagrams, activity diagrams using PlantUML/Mermaid, architecture-to-code traceability, and zero-ambiguity documentation standards. Owned by Rafael Okonkwo (Software Architect). Use during Stage 3 (UML Engineering) for UML package production and Stage 6 (Code Review) for architecture conformance. Trigger: UML engineering, class diagram, sequence diagram, component diagram, PlantUML, Mermaid, architecture traceability.
prerequisites:
