...
description: "iOS accessibility (a11y) implementation — VoiceOver optimization, Dynamic Type support, Switch Control compatibility, accessibility identifiers, and automated auditing. Owned by Seo-Yeon Park (iOS Lead). Use during Stage 5 (Development) for accessible UI construction and Stage 6 (Code Review) for a11y conformance audits. Trigger: ios accessibility, voiceover, dynamic type, switch control, accessibility label, accessibility identifier, wcag, a11y, touch target, content size category, accessibility inspector."
prerequisites:
  - ios-ui-ux-swiftui
