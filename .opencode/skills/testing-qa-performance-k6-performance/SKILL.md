...
description: k6 performance testing for mobile backend APIs — load testing, stress testing, spike testing, endurance testing, SLA validation (P99 latency, throughput, error rate), and performance benchmark reporting for mobile API services. Owned by Priya Sharma (SDET). Use during Stage 7 (Testing) for API performance benchmark execution and Stage 10 (Release Readiness) for performance SLA validation. Trigger: k6 performance testing, load testing, stress testing, spike testing, endurance testing, P99 latency, throughput testing, API performance benchmarks.
prerequisites:
  - testing-qa-overview
...
**Category:** Backend & API Test Engineering — Performance Testing
**Owner:** SDET Web/Backend #1 (Priya Sharma)

## Overview

This skill covers k6 load testing for performance engineering — scenario configuration, threshold definition, distributed execution, result analysis, bottleneck identification, and performance regression detection. k6 is a developer-centric load testing tool that uses JavaScript for test scripting, integrates natively with CI/CD pipelines, and produces machine-readable metrics for automated quality gates.

Performance testing is critical for Stage 7 automated testing to establish baselines, validate SLAs, and detect performance regressions before release. Unlike functional testing, performance tests measure how the system behaves under load — identifying bottlenecks, capacity limits, and degradation patterns that only manifest when multiple concurrent users interact with the system simultaneously.
