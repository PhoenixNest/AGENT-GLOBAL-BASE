---
name: backend-database-sharding
description: Database sharding for horizontal scalability — sharding key selection, hash-based and range-based distribution, distributed query execution, eventual consistency, automated failover, and live rebalancing without downtime. Owned by Dev Malhotra (Backend Chapter Lead). Use during Stage 3 (Architecture) for sharding strategy and Stage 5 (Development) for shard implementation. Trigger: database sharding, horizontal partitioning, sharding key, distributed queries, shard rebalancing, eventual consistency.
prerequisites:
