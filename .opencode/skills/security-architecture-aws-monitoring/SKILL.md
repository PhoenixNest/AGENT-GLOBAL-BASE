---
name: security-architecture-aws-monitoring
description: AWS security monitoring for mobile backend infrastructure — GuardDuty threat detection, Security Hub compliance dashboard, CloudTrail audit logging, IAM Access Analyzer, and automated incident response for mobile API services. Owned by Leila Khoury (Security Engineer). Use during Stage 5 (Development) for AWS security configuration and Stage 8 (Integrity Verification) for compliance validation. Trigger: AWS monitoring, GuardDuty, Security Hub, CloudTrail, IAM Access Analyzer, incident response, AWS security, mobile backend monitoring.
prerequisites:
  - security-overview
