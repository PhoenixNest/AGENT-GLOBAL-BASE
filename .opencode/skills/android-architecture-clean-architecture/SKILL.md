---
name: android-architecture-clean-architecture
description: Android clean architecture, MVVM, and MVI pattern guidance for native Android apps — layered architecture (presentation/domain/data), StateFlow state management, use case interactor design, repository pattern, process death recovery, and Hilt dependency injection. Owned by Tariq Al-Hassan (Senior Android Engineer). Use during Stage 5 (Development) for Android architecture decisions and Stage 6 (Code Review) for architectural conformance. Trigger: android architecture, clean architecture, MVVM, MVI, layered architecture, repository pattern, use case interactor, StateFlow, module structure, process death.
prerequisites:
