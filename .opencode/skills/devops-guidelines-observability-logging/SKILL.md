...
description: Observability and logging for mobile applications — OpenTelemetry instrumentation, structured logging standards, log aggregation (Datadog/New Relic), error tracking (Sentry), and real-user monitoring (RUM) for iOS and Android apps. Owned by Thomas Zhang (DevOps Lead). Use during Stage 5 (Development) for observability instrumentation and Stage 7 (Testing) for performance baseline validation. Trigger: observability, OpenTelemetry, structured logging, log aggregation, Datadog, New Relic, Sentry, error tracking, RUM, real-user monitoring, mobile app monitoring.
prerequisites:
  - devops-overview
