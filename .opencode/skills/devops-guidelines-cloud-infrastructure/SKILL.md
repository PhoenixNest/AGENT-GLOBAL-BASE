...
description: Cloud infrastructure for mobile backends — AWS/Azure/GCP service selection, serverless architecture (Lambda/Cloud Functions), managed databases (DynamoDB/RDS), CDN configuration, and infrastructure cost optimization for mobile API backends. Owned by Thomas Zhang (DevOps Lead). Use during Stage 3 (UML Engineering) for infrastructure architecture decisions and Stage 5 (Development) for cloud provisioning. Trigger: cloud infrastructure, AWS mobile backend, Azure mobile services, GCP backend, serverless, managed database, CDN, infrastructure cost optimization.
prerequisites:
  - devops-overview
