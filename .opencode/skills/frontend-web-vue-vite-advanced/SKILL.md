---
name: frontend-web-vue-vite-advanced
description: Vue 3 + Vite advanced patterns — Composition API architecture, Vite plugin development, HMR tuning, code splitting, tree shaking, and build optimization for production-grade Vue apps. Owned by Amira Voss (Frontend Chapter Lead). Use during Stage 5 (Development) for Vue implementation and Stage 6 (Code Review) for build performance review. Trigger: vue 3 advanced, vite plugin, composition api, hmr tuning, code splitting vue, tree shaking, vue build optimization.
prerequisites:
