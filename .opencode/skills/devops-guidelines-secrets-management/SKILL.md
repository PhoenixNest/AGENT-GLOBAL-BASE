...
description: Secrets management for mobile development — HashiCorp Vault integration, AWS Secrets Manager, CI/CD secret injection, runtime secret rotation, and secure credential handling for API keys, signing certificates, and OAuth tokens in Android/iOS pipelines. Owned by Thomas Zhang (DevOps Lead). Use during Stage 4 (Implementation Plan) for secrets architecture and Stage 5 (Development) for secret injection implementation. Trigger: secrets management, HashiCorp Vault, AWS Secrets Manager, CI/CD secrets, secret rotation, API keys, signing certificates, OAuth tokens, secure credentials.
prerequisites:
  - devops-guidelines-network-security-fundamentals
