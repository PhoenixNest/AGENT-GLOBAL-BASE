...
description: PostgreSQL database architecture at scale — table partitioning strategies, read replica topologies, PgBouncer connection pooling, automated migration pipelines, and distributed data consistency patterns. Owned by Dev Malhotra (Backend Chapter Lead). Use during Stage 3 (Architecture) for database design decisions and Stage 5 (Development) for schema implementation. Trigger: database architecture, postgresql scaling, table partitioning, read replicas, pgbouncer, database migrations, data consistency.
prerequisites:
