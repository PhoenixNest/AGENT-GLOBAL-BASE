---
name: frontend-web-performance-security-frontend-security
description: Frontend security engineering — Content Security Policy implementation, XSS prevention with DOMPurify, OAuth 2.0 PKCE flows, secure storage patterns, and security review protocols for Stage 6/8 gates. Owned by Amira Voss (Frontend Chapter Lead). Use during Stage 5 (Development) for secure implementation and Stage 6 (Code Review) for security validation. Trigger: frontend security, csp, content security policy, xss prevention, dompurify, oauth pkce, secure storage, security review.
prerequisites:
