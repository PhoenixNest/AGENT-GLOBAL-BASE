---
name: android-testing-quality-test-infra
description: Android test infrastructure — test fixtures and factory patterns, CI test orchestration with GitHub Actions, parallel test execution and sharding, flaky test detection and quarantine, MockWebServer setup, and coroutine test utilities. Owned by Nina Bergström (Android Engineer). Use during Stage 5 (Development) for test utility creation and Stage 7 (Automated Testing) for full test suite execution. Trigger: android test infrastructure, test fixtures, factory pattern, CI testing, parallel tests, flaky test, mockwebserver, test utilities, coroutine testing.
prerequisites:
  - android-testing-quality-testing
