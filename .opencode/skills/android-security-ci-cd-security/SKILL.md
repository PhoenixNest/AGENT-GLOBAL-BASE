---
name: android-security-ci-cd-security
description: Android security engineering — OWASP MASVS L1/L2 compliance, Android Keystore cryptography, certificate pinning with OkHttp, EncryptedSharedPreferences, SQLCipher, R8/ProGuard obfuscation, root detection, and data leakage prevention. Owned by Priya Narayanan (Senior Android Engineer). Use during Stage 5 (Development) for secure implementation and Stage 8 (Integrity Verification) for MASVS compliance validation. Trigger: android security, OWASP MASVS, keystore, certificate pinning, encryption, R8, proguard, obfuscation, root detection, data protection, secure storage.
prerequisites:
