---
name: cross-platform-kmp-shared-modules
description: KMP shared module design and implementation — domain layer sharing, data layer abstraction, platform adapter interfaces, contract verification, and cross-platform API consistency for iOS and Android consumers. Owned by Mei-Ling Johansson (Cross-Platform Lead). Use during Stage 5 (Development) for KMP shared module development and Stage 6 (Code Review) for contract verification. Trigger: KMP shared modules, shared domain layer, KMP contract verification, platform adapters, cross-platform API consistency.
prerequisites:
