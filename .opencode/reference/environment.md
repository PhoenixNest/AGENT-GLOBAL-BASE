...
| OpenCode CLI | Local installation                          | Agents and skills loaded from `.opencode/` directory |
...
## Hook Scripts (Not Migrated — Untested)

**Source Location:** `.qwen/hooks/company/automated-recruitment-pipeline/scripts/`

**Note:** Hook scripts were NOT migrated to `.opencode/` as they are untested. They remain in `.qwen/` for reference only.

**Count:** 23 Python files (in `.qwen/` source)

**Test:** `python .qwen/hooks/company/automated-recruitment-pipeline/tests/run-all-tests.py`
