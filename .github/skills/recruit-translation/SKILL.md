...
8. **Placement recommendation**
   - Recommended tier: `supervisors/` or `teammates/`
...
1. Create `[tier]/[role-name]/agent/profile.md` — where `[tier]` is either `supervisors` or `teammates` based on seniority (see CHRO Seniority → Tier Mapping table) — using the Recruited Agent Profile Template from the CHRO profile
2. Create at least one `[tier]/[role-name]/skills/[skill-name].md` covering their primary translation capability
3. Confirm to the user: "Recruited and placed: [Name], [Title] → localization/[tier]/[role-name]/"
