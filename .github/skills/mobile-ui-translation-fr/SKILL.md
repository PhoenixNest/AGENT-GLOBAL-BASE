---
name: mobile-ui-translation-fr
description: Mobile UI string translation and quality assurance for French. Covers Language Translation Module workflow, character limit compliance, platform writing conventions (iOS HIG, Android Material Design), register calibration, MT post-editing, and linguistic QA gate before Translation Verification Report.
---

# Mobile UI Translation (French)

## Purpose

Translate mobile UI strings from the EN-source handoff package into French, produce correctly structured platform resource files, and pass the linguistic QA gate before delivery to the Chief Translation Officer for the Translation Verification Report.
...
- French typically runs 20–30% longer than English — apply approved brevity techniques; escalate to the CTO-L if a limit is genuinely unachievable
...
- iOS: respect Apple's localization guidelines for French
- Android: respect Material Design writing guidelines for French
...
French requires `one` and `other` plural forms. FR-CA follows the same plural rules as FR-FR.
