...
| **No dedicated safety/eval function**                   | ASGF compliance execution now sits with Farouk (correctly, as a delegated _execution_ role), but there is no crew member whose sole mandate is red-teaming, adversarial evaluation, or safety regression testing across all four modules. Right now that's still entirely on me.                                                                                                                                                                                                                |
...
of them block current operations: every module has an owner today, and I retain ASGF ratification,
