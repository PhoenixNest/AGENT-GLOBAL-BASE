# core-component-00/platform/model-context-protocol-servers/ — CC-00 MCP Server Implementations

Production MCP server implementations exposed to Claude Code. This is the lab's deployment
surface — architecturally distinct from the five-module research stack under `framework/`, though
it depends on it.
...
core-component-00/platform/model-context-protocol-servers/<server>/.venv/bin/python -m pip install "torch==2.13.0+cu130" --index-url https://download.pytorch.org/whl/cu130
```

```powershell
# Windows — repeat for each server directory
core-component-00\platform\model-context-protocol-servers\<server>\.venv\Scripts\python.exe -m pip install "torch==2.13.0+cu130" --index-url https://download.pytorch.org/whl/cu130
```
