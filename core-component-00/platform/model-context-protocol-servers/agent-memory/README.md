...
_working_ — see `../../framework/02-context-engineering/CLAUDE.md`); three of the four names overlap but
...
(`../../framework/02-context-engineering/memory/episodic/<session_id>.jsonl`, `semantic.jsonl`,
...
`../../framework/02-context-engineering/implementations/memory_vector_store.py` (and, once a
...
tool. The engineering lives in `../../framework/02-context-engineering/`; this server is the thin MCP
...
`core-component-00/platform/model-context-protocol-servers/.venv/`, together with `workspace-knowledge` and
...
would depend on whichever server started it first. See `../CLAUDE.md` § Python Environment
...
```bash
# Linux/macOS
core-component-00/platform/model-context-protocol-servers/.venv/bin/python -m pip install -e core-component-00/platform/model-context-protocol-servers/agent-memory
```

```powershell
# Windows
core-component-00\platform\model-context-protocol-servers\.venv\Scripts\python.exe -m pip install -e core-component-00\platform\model-context-protocol-servers\agent-memory
```
...
python core-component-00/platform/model-context-protocol-servers/_shared/provision_model.py sentence-transformers/all-MiniLM-L6-v2
...
python core-component-00/platform/model-context-protocol-servers/_shared/warm_embedder_cache.py
```
...
name resolved via `PATH` (see `core-component-00/platform/model-context-protocol-servers/CLAUDE.md` § Python Environment for
...
`core-component-00/platform/model-context-protocol-servers/_shared/models/sentence-transformers--all-MiniLM-L6-v2/` via
...
this environment; it is now resolved via a lazy embedder warmup. Full status is tracked in
...
1. The embedder background-load stall described above is resolved — see
   `.claude/rules/mcp-governance.md`'s `agent-memory` row.
