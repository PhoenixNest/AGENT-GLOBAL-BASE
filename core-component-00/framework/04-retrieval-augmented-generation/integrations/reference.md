# RAG Integrations Reference

## Complete Integration Specifications for Enterprise Data Sources

This reference document provides integration specifications, connectors, and patterns for connecting various data sources to your RAG system.

---

## Quick Reference Matrix

| Data Source             | Complexity | Connector Type        | Format Support         | Real-time Sync    | Recommended For           |
| ----------------------- | ---------- | --------------------- | ---------------------- | ----------------- | ------------------------- |
| **Markdown files**      | Easy       | File watcher parser   | `.md`, `.txt`          | Polling (30s)     | Documentation wikis       |
| **PDF documents**       | Medium     | PDF parser API        | PDF with text layer    | Batch only        | Technical manuals         |
| **Git repositories**    | Easy       | Git LFS + hooks       | Source files, READMEs  | Near real-time    | Developer documentation   |
| **Confluence/Wiki**     | Medium     | REST API connector    | Multiple formats       | Webhook / polling | Enterprise knowledge base |
| **SharePoint/OneDrive** | Medium     | Azure integration     | `.docx`, `.pdf`, `.md` | Polling (5min)    | Corporate documents       |
| **REST API endpoints**  | Hard       | HTML scraper + parser | HTML → Markdown        | Webhook / polling | Public facing docs        |
| **Database schemas**    | Easy       | SQL query connector   | Structured metadata    | CDC / batch       | Data documentation        |
...
```python
"""
markdown_ingestor.py - Ingest markdown files into RAG system
"""
import asyncio
...
class MarkdownIngestor:
    ...
    async def ingest_directory(self, directory_path: str, recursive: bool = True) -> int:
        ...
        for md_file in md_files:
            ...
            # Chunk the content (500 tokens, 128 overlap)
            chunks = self._split_into_chunks(content, chunk_size=512, overlap=128)

            for i, chunk in enumerate(chunks):
                # Create embedding
                embedding_vector = await self.embedding.embed(chunk["text"])
...
### Heading-Based Chunking for Documentation
...
def chunk_markdown_by_headings(content: str, heading_levels: List[int] = [1, 2, 3]) -> List[Dict]:
    ...
    chunks = []
    current_section = {"text": "", "headings": [], "level": 0}

    for line in content.split("\\n"):
        for level, pattern in [(h, rf'^#{h}[ \\n]+([^\\n]+)') for h in heading_levels]:
            if re.match(pattern, line):
                # Save previous section
                if current_section["text"].strip():
                    chunks.append({
                        "text": current_section["text"],
                        "headings": current_section["headings"],
                        "heading_level": current_section.get("level", 1)
                    })
                ...
                current_section = {
                    "headings": [line],
                    "text": "",
                    "level": h if (h := int(line.split("#")[0].strip())) else 3
                }
    ...
    if current_section["text"].strip():
        chunks.append({
            "text": current_section["text"],
            "headings": current_section["headings"],
            "heading_level": current_section.get("level", 3)
        })
...
| Parser             | Accuracy  | Speed  | Best Use Case            | Dependencies              |
| ------------------ | --------- | ------ | ------------------------ | ------------------------- |
| **PyMuPDF (fitz)** | High      | Fast   | General PDF processing   | `pip install PyMuPDF`     |
| **pdfplumber**     | High+     | Medium | Tables + text extraction | `pip install pdfplumber`  |
| **pymupdf4llm**    | Optimized | Fast   | LLM-focused parsing      | `pip install pymupdf4llm` |
...
class PDFIngestor:
    ...
    async def ingest_pdf(
        ...
        for page_num in valid_pages:
            ...
            # Chunk the content
            chunked_content = self._chunk_pdf_content(content, chunk_size=512)
...
### Git Repository Parser with Commit Awareness

| Feature                | Complexity | Performance Impact    | Recommended Approach              |
| ---------------------- | ---------- | --------------------- | --------------------------------- |
...
### REST API Connector for Atlassian Confluence
...
class ConfluenceIngestor:
    ...
    async def _ingest_page(self, page_id: str, headers: Dict[str, str]) -> int:
        ...
        # Chunk the content
        chunks = self._chunk_markdown(markdown_content, chunk_size=512)
...
integrations:
  markdown_files:
    enabled: true
    source_paths:
      - "./core-component-00/framework/03-harness-engineering"
      - "./core-component-00/framework/01-prompt-engineering"
