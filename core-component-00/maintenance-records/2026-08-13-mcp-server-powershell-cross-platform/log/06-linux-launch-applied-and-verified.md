...
Pipeline stage 3 — Execution (`core-component-00/maintenance-records/pipeline.md`), on the existing
...
`${CLAUDE_PROJECT_DIR:-.}/core-component-00/mcp-servers/.venv/Scripts/python.exe` — the Windows-only
path, per the 2026-08-13 revert (`log/03-incident-revert.md`). No `.venv` existed at
`core-component-00/mcp-servers/.venv/` on this machine at all.

**Actions taken:**

1. Inspected this machine directly rather than applying the documented one-line edit blind:
   confirmed no shared venv exists at `core-component-00/mcp-servers/.venv/`. Instead, each server
...
2. Edited `.mcp.json`'s `"command"` for both servers to point at each server's own per-server venv
   ...
   - `workspace-knowledge`: `${CLAUDE_PROJECT_DIR:-.}/core-component-00/mcp-servers/workspace-knowledge/.venv/bin/python`
   - `agent-memory`: `${CLAUDE_PROJECT_DIR:-.}/core-component-00/mcp-servers/agent-memory/.venv/bin/python`
...
`core-component-00/mcp-servers/.venv/`, with an explicit rationale (deterministic `embedder-service`
