...
Pipeline stage 3→1 — Reopen (`core-component-00/maintenance-records/pipeline.md`): a new problem
found during Execution routes back to Investigation rather than closing.

**Trigger:** Immediately after
`core-component-00/maintenance-records/2026-08-13-mcp-server-powershell-cross-platform/log/02-remediation.md`'s
...
3. Reverted `.mcp.json`'s `"command"` for both servers back to the direct, absolute interpreter
   path (`${CLAUDE_PROJECT_DIR:-.}/core-component-00/mcp-servers/.venv/Scripts/python.exe`),
...
`core-component-00/maintenance-records/2026-08-13-mcp-server-powershell-cross-platform/maintenance-record.md`'s
