...
| 10  | **Disaster recovery / degradation stack** | Four-level fallback if the search index is unavailable: full hybrid search → an in-process backup index → keyword-only search over the raw log → a plain scan of the raw log; the plain-log write always succeeds regardless of index health | Extends the document-RAG module's existing multi-tier fallback design to memory; because the log is already the source of truth, the last-resort fallback exists for free                                                                      | Direct precedent: `architecture/overview.md` § 11; `evaluation/reference-table.md` § Orphaned Point Detection                                                          | [05-disaster-recovery-and-resilience.md](05-disaster-recovery-and-resilience.md)                                                                                                                                         | **❌ Only 2 of 4 levels exist.** The primary hybrid-search level and the last-resort raw-log-scan/rebuild level are both real and working. The two middle levels — an in-process backup index and a keyword-only fallback — were never built; if the primary search index is unavailable, retrieval currently has no automatic middle ground and falls straight to a manual rebuild. See [05-disaster-recovery-and-resilience.md](05-disaster-recovery-and-resilience.md) § 3 for the corrected picture.                                                      |
