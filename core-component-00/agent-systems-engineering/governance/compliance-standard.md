...
| **Prompt → Harness**        | Layer 1 persona/domain-resolution patterns (P-013 Persona Resolution, P-014 Delegation Routing — `engineering/prompt-engineering/patterns/advanced-patterns.md`) are authored once and consumed by any Layer 3 gate that triggers on persona or domain cues. The gate stays a heuristic trigger; identity/ownership resolution defers to the pattern, never freehanded in the hook itself. |
