...
_AGENT_MEMORY_SERVER_PATH = (
    Path(__file__).resolve().parents[3] / "mcp-servers" / "agent-memory" / "server.py"
)
...
_CONTEXT_ENGINEERING_IMPLEMENTATIONS_DIR = (
    Path(__file__).resolve().parents[3] / "engineering" / "context-engineering" / "implementations"
)
_HARNESS_ENGINEERING_IMPLEMENTATIONS_DIR = (
    Path(__file__).resolve().parents[3] / "engineering" / "harness-engineering" / "implementations"
)
