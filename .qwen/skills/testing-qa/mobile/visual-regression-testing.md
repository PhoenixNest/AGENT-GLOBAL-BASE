...
Visual regression testing detects unintended UI changes by comparing screenshots of the application under test against a known-good baseline ("golden master"). Unlike functional testing which validates behavior, visual regression testing validates *appearance* — pixel-level fidelity to the expected visual state.
...
update-baseline:
  ...
  rules:
    - if: '$CI_PIPELINE_SOURCE == "merge_request_event"'
      ...
      allow_failure: false
      variables:
        $VISUAL_REGRESSION_APPROVED == "true"
```
