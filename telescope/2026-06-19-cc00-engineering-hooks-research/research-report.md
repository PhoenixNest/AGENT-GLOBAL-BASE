...
| **Corrections Log**  | 2026-07-01 — the "Blocking?" column for H-P01 below (originally "No (inject)") describes that hook's state as of commit `031f4f3`; it is no longer current. See `telescope/2026-06-30-prompt-optimizer-audit/research-report.md` Pass 8: H-P01 gained real `PreToolUse`/`PostToolUse` blocking enforcement via two companion hooks (`prompt-gate-enforcer`/`prompt-gate-clear`) on 2026-07-01. Table left as an accurate historical snapshot rather than rewritten |
