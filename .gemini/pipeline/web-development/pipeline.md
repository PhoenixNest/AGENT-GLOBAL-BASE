...
- **8 mandatory ADRs**:
  ...
  - `ADR-OBSERVABILITY.md` — Frontend error tracking (Sentry/DataDog RUM), performance monitoring (Lighthouse CI + synthetic monitoring), user journey analytics correlation, API call tracing integration with backend OpenTelemetry
...
1. All **8** ADRs authored, reviewed, and approved
...
| 4   | Security — SRD enforced, web security controls effective | CSO                | All security controls present AND effective (XSS prevention, CSRF protection, CSP headers with strict-dynamic/nonces, OAuth 2.0 session integrity, **Subresource Integrity hashes verified for all third-party scripts**), stealthy weakening verified absent |
