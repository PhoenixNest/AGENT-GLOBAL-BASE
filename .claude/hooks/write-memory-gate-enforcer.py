...
# § Write-Path Security.
#
# Same PreToolUse/PostToolUse hook-pair shape as H-P01 (prompt-gate-enforcer.py /
# prompt-gate-clear.py) — deliberately reused, not reinvented. This is a NEW, INDEPENDENT pair: it
# checks its own marker file (see MARKER_PREFIX below), never H-P01's
# "h-p01-pending-<session_id>.json", and does not modify or depend on prompt-gate-enforcer.py/
# prompt-gate-clear.py in any way.
#
# Wired live in .claude/settings.json (PreToolUse, matcher "*", alongside prompt-gate-enforcer.py).
# See write-memory-gate-clear.py for the matching PostToolUse entry.
#
# Honest scope/limitation: this hook provides a structurally-enforced confirmation gate for writes
...
# all. Per REFLECT-003, no purely code-level check — this hook included — can be *the* security
# boundary against a determined bypass; it narrows the failure window for the one real, in-scope
# caller this workspace has today, and is defense-in-depth, not an unforgeable gate.
#
