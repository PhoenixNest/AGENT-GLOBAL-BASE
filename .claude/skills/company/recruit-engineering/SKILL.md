---
name: company-recruit-engineering
description: Engineering role recruitment — Interview Simulation Protocol for engineers (mobile, backend, platform, infrastructure). Generates candidate profiles with specific companies, outcomes, and scale. Owned by Dr. Evelyn Hartwell (CHRO).
...
6. **Vetting result** — apply `vet-candidate` skill and paste the full scoring output
...
1. Create `team/[tier]/[role-name]/agent/profile.md` — where `[tier]` is either `supervisors` or `teammates` based on seniority.
