---
name: company-application-security-hardening
description: Application security hardening for mobile — code obfuscation (ProGuard, DexGuard), anti-tampering, root/jailbreak detection, SSL pinning, Runtime Application Self-Protection (RASP), reverse engineering prevention. Owned by Dr. Sarah Chen (CSO).
...
**iOS Protection:** iXGuard, Arxan, Promon SHIELD; custom LLVM obfuscation passes; Swift obfuscation techniques.

**Android Protection:** DexGuard, Guardsquare, Arxan; ProGuard/R8 optimization; native library protection; SafetyNet/Play Integrity Attestation API.

**Cross-Platform:** TrustKit for certificate pinning; custom RASP frameworks; device fingerprinting.
